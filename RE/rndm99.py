...
def dfs_first_match_random(node, visited=None, parent_value=""):
    if visited is None:
        visited = set()
    visited.add(node)
    ...
    if node.value.startswith("Jam"):
        if node.children:
            ...
            return f"{parent_value}, {node.value} dengan {pilihan.value}"
            
    for child in node.children:
        if child not in visited:
            hasil = dfs_first_match_random(child, visited, parent_value)
            if hasil: 
                return hasil
    return None
...
hasil = dfs_first_match_random(root_node)
if hasil:
    print(f"Hasil: {hasil}")
else:
    print("")
